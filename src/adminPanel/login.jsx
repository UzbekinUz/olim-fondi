...
export default function Login({ setAdmin, handleRef }) {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      // Backendga so'rov yuborish (Siz yozgan res.send({ok, msg, data}) formatiga moslangan)
      const response = await axios.post(`${API_LINK}/admin/signin`, {
        username,
        password,
      });
      ...
      if (resData.ok) {
        addToast("Tizimga muvaffaqiyatli kirdingiz!", "success");// Tokenni localStorage'ga saqlash
        localStorage.setItem("access_admin_token", resData.access_token);

        // Admin holatini yangilash va sahifani yo'naltirish
        setTimeout(() => {
         handleRef();
        }, 1000);

      } else {
        // Agar backend error qaytarsa (masalan: "Bunday username mavjud emas")
        triggerError(resData.msg || "Xatolik yuz berdi");
      }
    } catch (err) {
      console.error(err);
      triggerError("Server bilan aloqa uzildi yoki xatolik yuz berdi!");
    } finally {
      setLoading(false);
    }
  };

  // Xatolik yuz berganda animatsiya va xabarlarni ishga tushirish
  const triggerError = (msg) => {
    setError(msg);
    addToast(msg, "error");
    setShake(true);
    setTimeout(() => setShake(false), 400); // 400ms dan keyin shake klassini o'chirish
  };
  ...
  return (
    <div className="min-h-screen flex flex-col justify-between overflow-x-hidden bg-slate-900 text-slate-100 selection:bg-teal-500 selection:text-slate-950 transition-colors duration-300 font-sans relative">
      ...
      <main className="grow flex items-center justify-center px-4 sm:px-6 lg:px-8 py-16 relative">
        ...
        <div className={`w-full max-w-md transition-all duration-500 ease-out transform ${shake ? "animate-shake" : ""}`}>
          <div className="glass-container border border-slate-800/80 shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5)] rounded-3xl p-8 sm:p-10 relative overflow-hidden">
            ...
            <div className="absolute top-0 left-0 right-0 h-1 bg-linear-to-r from-teal-400 via-emerald-400 to-indigo-500"></div>
              ...
              <div>
                ...
                <div className="relative rounded-xl shadow-sm">
                  ...
                  <input
                    ...
                    className="block w-full pl-11 pr-4 py-3.5 border border-slate-800 rounded-xl bg-slate-950/50 text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-teal-500/50 focus:border-teal-400 transition-all duration-200 text-sm disabled:opacity-50"
              ...
              <div>
                ...
                <div className="relative rounded-xl shadow-sm">
                  ...
                  <input
                    ...
                    className="block w-full pl-11 pr-12 py-3.5 border border-slate-800 rounded-xl bg-slate-950/50 text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-teal-500/50 focus:border-teal-400 transition-all duration-200 text-sm disabled:opacity-50"
                    placeholder="••••••••"
      ...
      <div className="fixed bottom-5 right-5 z-50 flex flex-col gap-3 max-w-sm w-full px-4 sm:px-0">
        {toasts.map((toast) => {
          // ESLint muammosini keltirib chiqargan o'zgaruvchilarni render ichida dinamik aniqlaymiz
          const isSuccess = toast.type === "success";
          const isError = toast.type === "error";

          const colorClasses = isSuccess 
            ? "border-emerald-500/20 text-emerald-300 toast-glass shadow-[0_10px_30px_rgba(16,185,129,0.1)]"
            : isError 
            ? "border-red-500/20 text-red-300 toast-glass shadow-[0_10px_30px_rgba(239,68,68,0.1)]"
            : "border-indigo-500/20 text-indigo-300 toast-glass shadow-[0_10px_30px_rgba(99,102,241,0.1)]";
